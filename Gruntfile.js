  ...
  grunt.initConfig({
    ...
    watch: {
      ...
      coffeeGame: {
        ...
        tasks: ['newer:coffee:compileGame']
      },
      injectJS: {
    ...
    coffee: {
      ...
      compileServer: {
        ...
        files: [{
          expand: true,
          cwd: yeoman.srcpath.coffeeServer.cwd,
          src: yeoman.srcpath.coffeeServer.src,
    ...
    ngconstant: {
      ...
      dev: {
        constants: {
          config: require('./public/config/config.dev')
        }
      },
      prod: {
        constants: {
          config: require('./public/config/config.production')
        }
      }
    },
    ...
    replace: {
      prod: {
        options: {
          patterns: [{
            match: /<(html_disabled)>/i,
            replacement: '<$1 manifest="manifest.appcache">'
          }, {
            match: /\s+<script[^>]+?livereload\.js[\s\S]+?><\/script>\s+/i,
            replacement: ' '
          }]
        },
        files: {
